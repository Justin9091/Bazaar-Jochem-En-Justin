...
@section("main-content")
    @if ($ad->type == "rent")
        <div class="p-3">
            <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onclick="openPopup({{ $ad->id }})">@lang('ad.rent_item')</button>
        </div>
            ...
            <div class="bg-gray-600 rounded-lg p-8 relative">
                ...
                <x-form id="rentForm" action="{{ route('rentitem', ['id' => $ad->id]) }}" method="POST">
                    ...
                    <div class="flex justify-center">
                        <button type="submit" class="flex items-center bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded border border-blue-700">@lang('ad.submit')</button>
                    </div>
                </x-form>
            </div>
        </div>
    ...
    <x-review-list :userid="$ad->user_id" :adid="$ad->id" :reviews="$ad->getReviews($ad->id, $ad->user_id)"/>
